...
RUN cd /tmp && wget "https://apt.llvm.org/llvm.sh" && chmod +x llvm.sh && ./llvm.sh ${LLVM_VERSION} all && apt-get install liblld-13-dev -y && rm llvm.sh
RUN update-alternatives \
    --install /usr/bin/llvm-config       llvm-config      /usr/bin/llvm-config-${LLVM_VERSION} 1 \
...
RUN update-alternatives \
    --install   /usr/bin/clang++         clang++          /usr/bin/clang++-${LLVM_VERSION} 1
RUN update-alternatives \
    --install   /usr/bin/lld             lld              /usr/bin/lld-${LLVM_VERSION} 1 \
