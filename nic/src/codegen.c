...
void ni_ast_codegen_node_var(ni_ast_node *ast_node) {
  ...
  LLVMValueRef addr = LLVMBuildAlloca(
      builder, type, ast_node->variable_declaration.identifier);
...
void ni_ast_codegen_node_call(ni_ast_node *ast_node) {
  ...
  if (ast_node->function_call.argument_list != NULL) {
    ...
    ni_ast_node_list_entry *current = ast_node->function_call.argument_list->head;
    int i = 0;
    while (current != NULL) {
  ...
  symbol_value *function = get_symbol_table(fn_table, ast_node->function_call.identifier);
...
void ni_ast_codegen_node_fun(ni_ast_node *ast_node) {
  ...
  insert_symbol_table(fn_table, ast_node->function_declaration.identifier, entry);
  ...
  if (ast_node->function_declaration.argument_list != NULL) {
    ...
    ni_ast_node_list_entry *current = ast_node->function_declaration.argument_list->head;
    int i = 0;
    while (current != NULL) {
      LLVMTypeRef type = LLVMInt32Type();
      LLVMValueRef addr = LLVMBuildAlloca(
          builder, type, current->node->argument.identifier);
