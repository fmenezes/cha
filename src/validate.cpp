...
bool TypeUtils::is_float(PrimitiveType type) {
    return type == PrimitiveType::FLOAT16 ||
           type == PrimitiveType::FLOAT32 ||
           type == PrimitiveType::FLOAT64 ||
           type == PrimitiveType::CONST_FLOAT;
}

PrimitiveType TypeUtils::convert_arithmetic_types(PrimitiveType left, PrimitiveType right) {
...
bool TypeUtils::is_assignment_compatible(PrimitiveType from, PrimitiveType to) {
    ...
    if (from == PrimitiveType::CONST_INT && is_signed_int(to)) return true;
    if (from == PrimitiveType::CONST_UINT && is_unsigned_int(to)) return true;
    if (from == PrimitiveType::CONST_FLOAT && is_float(to)) return true;
    ...
    if (is_numeric(from) && is_numeric(to)) {
        ...
        if (is_signed_int(from) && is_signed_int(to)) return true;
        if (is_unsigned_int(from) && is_unsigned_int(to)) return true;
...
bool TypeUtils::is_numeric_comparison_compatible(PrimitiveType left, PrimitiveType right) {
...
bool TypeUtils::is_equality_comparison_compatible(PrimitiveType left, PrimitiveType right) {
    ...
    if (node.result_type() && node.result_type()->is_primitive()) {
        ...
        if (current == PrimitiveType::CONST_INT ||
            ...
            node.set_result_type(std::make_unique<AstType>(
                node.location(), AstType::Primitive(type)));
        }
    }
}
...
Validator::Validator() 
    : symbol_table_(std::make_shared<SymbolTable>()), current_function_(nullptr) {}
...
bool Validator::validate_top_level(const AstNodeList& nodes) {
    ...
    for (const auto& node : nodes) {
        if (auto func_decl = dynamic_cast<const FunctionDeclarationNode*>(node.get())) {
...
bool Validator::validate_node(const AstNode& node) {
    ...
    } else if (auto const_decl = dynamic_cast<const ConstantDeclarationNode*>(&node)) {
        return validate_constant_declaration(*const_decl);
    } else if (auto var_decl = dynamic_cast<const VariableDeclarationNode*>(&node)) {
    ...
    } else if (auto var_assign = dynamic_cast<const VariableAssignmentNode*>(&node)) {
        return validate_variable_assignment(*var_assign);
    } else if (auto var_lookup = dynamic_cast<VariableLookupNode*>(const_cast<AstNode*>(&node))) {
        return validate_variable_lookup(*var_lookup);
    } else if (auto bin_op = dynamic_cast<BinaryOpNode*>(const_cast<AstNode*>(&node))) {
        return validate_binary_op(*bin_op);
    } else if (auto func_call = dynamic_cast<FunctionCallNode*>(const_cast<AstNode*>(&node))) {
...
bool Validator::validate_function_declaration(const FunctionDeclarationNode& node) {
    create_stack_frame();
...
bool Validator::validate_constant_declaration(const ConstantDeclarationNode& node) {
...
bool Validator::validate_variable_declaration(const VariableDeclarationNode& node) {
    bool success = true;
...
bool Validator::validate_variable_assignment(const VariableAssignmentNode& node) {
    ...
    if (auto var_decl = dynamic_cast<const VariableDeclarationNode*>(entry->node.get())) {
        if (!check_type_assignment(const_cast<AstNode&>(node.value()), var_decl->type())) {
            add_error(node.location(),
                     "type mismatch expects '" + TypeUtils::type_to_string(&var_decl->type()) + 
                     "' passed '" + TypeUtils::type_to_string(node.value().result_type()) + "'");
            success = false;
        }
    }
...
bool Validator::validate_variable_lookup(VariableLookupNode& node) {
    ...
    if (auto const_decl = dynamic_cast<const ConstantDeclarationNode*>(entry->node.get())) {
    ...
    } else if (auto var_decl = dynamic_cast<const VariableDeclarationNode*>(entry->node.get())) {
        // Copy type from variable declaration
        node.set_result_type(var_decl->type().clone());
    } else if (auto arg_node = dynamic_cast<const ArgumentNode*>(entry->node.get())) {
...
bool Validator::validate_binary_op(BinaryOpNode& node) {
    ...
    if (!left_type || !right_type || !left_type->is_primitive() || !right_type->is_primitive()) {
    ...
    switch (node.op()) {
        ...
        case Operator::DIVIDE: {
            PrimitiveType result_type = TypeUtils::convert_arithmetic_types(left_prim, right_prim);
            ...
            node.set_result_type(std::make_unique<AstType>(node.location(), AstType::Primitive(result_type)));
            TypeUtils::set_type_on_const(const_cast<AstNode&>(node.left()), result_type);
            TypeUtils::set_type_on_const(const_cast<AstNode&>(node.right()), result_type);
            break;
        }
        ...
        case Operator::LESS_THAN_OR_EQUALS: {
            ...
            node.set_result_type(std::make_unique<AstType>(node.location(), AstType::Primitive(PrimitiveType::BOOL)));
            TypeUtils::set_type_on_const(const_cast<AstNode&>(node.left()), right_prim);
            TypeUtils::set_type_on_const(const_cast<AstNode&>(node.right()), left_prim);
            break;
        }
...
bool Validator::validate_function_call(FunctionCallNode& node) {
    const SymbolEntry* entry = symbol_table_->lookup(node.identifier());
    if (!entry) {
        add_error(node.location(), "function '" + node.identifier() + "' not found");
        return false;
    }
    
    auto func_decl = dynamic_cast<const FunctionDeclarationNode*>(entry->node.get());
    if (!func_decl) {
    ...
    for (size_t i = 0; i < node.arguments().size(); ++i) {
        ...
        auto arg_decl = dynamic_cast<const ArgumentNode*>(func_decl->arguments()[i].get());
        if (arg_decl && !check_type_assignment(*node.arguments()[i], arg_decl->type())) {
            add_error(node.arguments()[i]->location(),
                     "type mismatch expects '" + TypeUtils::type_to_string(&arg_decl->type()) +
                     "' passed '" + TypeUtils::type_to_string(node.arguments()[i]->result_type()) + "'");
            success = false;
        }
    }
...
bool Validator::validate_function_return(const FunctionReturnNode& node) {
    ...
    if (!node.value() && current_function_->return_type().is_primitive() && 
        current_function_->return_type().as_primitive().type != PrimitiveType::UNDEF) {
    ...
    if (node.value() && !check_type_assignment(const_cast<AstNode&>(*node.value()), current_function_->return_type())) {
        add_error(node.location(),
                 "return type mismatch expects '" + TypeUtils::type_to_string(&current_function_->return_type()) +
                 "' passed '" + TypeUtils::type_to_string(node.value()->result_type()) + "'");
        success = false;
    }
...
bool Validator::validate_if(const IfNode& node) {
    ...
    if (node.condition().result_type() && node.condition().result_type()->is_primitive()) {
        if (node.condition().result_type()->as_primitive().type != PrimitiveType::BOOL) {
...
bool Validator::check_type_assignment(AstNode& value_node, const AstType& expected_type) {
...
void Validator::add_error(const AstLocation& location, const std::string& message) {
