...
void Logger::log(LogLevel level, const AstLocation &location,
...
std::string Logger::format_location(const AstLocation &location) const {
  std::stringstream ss;
  ss << location.file << ":" << location.line_begin << ":"
...
void log_validation_error(const AstLocation &location,
                          const std::string &message) {
  Logger::instance().error(location, message);
}
...
void log_info(const std::string &message) { Logger::instance().info(message); }
