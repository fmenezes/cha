...
void ni::ast::validator::visit(const ni::ast::function_call &node) {
  ...
  if (it == functions.end()) {
    throw ni::ast::syntax_error(node.loc, "function \"" + node.identifier +
                                              "\" not defined");
  }
  if (node.params.size() != it->second.args.size()) {
    throw ni::ast::syntax_error(
...
void ni::ast::validator::visit(const ni::ast::variable_lookup &node) {
  ...
  if (it == vars.end()) {
    throw ni::ast::syntax_error(node.loc, "variable \"" + node.identifier +
                                              "\" not defined");
  }
...
void ni::ast::validator::visit(const ni::ast::function_declaration &node) {
  ...
  for (auto &arg : node.args) {
    ...
    if (it != this->vars.end()) {
      throw ni::ast::syntax_error(arg->loc, "argument \"" + arg->identifier +
                                                "\" already defined (at " +
                                                it->second.str() + ")");
    }
...
void ni::ast::validator::visit(const ni::ast::program &node) {
  for (auto &n : node.instructions) {
    ...
    if (it != this->functions.end()) {
      throw ni::ast::syntax_error(n->loc, "function \"" + n->identifier +
                                              "\" already defined (at " +
                                              it->second.loc.str() + ")");
    }
