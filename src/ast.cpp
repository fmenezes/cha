...
AstTypePtr AstType::clone() const {
    if (is_primitive()) {
        ...
        return std::make_unique<AstType>(location_, 
            Array{arr.element_type->clone(), arr.size});
    } else {
...
AstNodePtr ConstantUnsignedIntegerNode::clone() const {
    auto cloned = std::make_unique<ConstantUnsignedIntegerNode>(location(), value_);
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr BinaryOpNode::clone() const {
    auto cloned = std::make_unique<BinaryOpNode>(location(), op_, left_->clone(), right_->clone());
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr VariableAssignmentNode::clone() const {
    auto cloned = std::make_unique<VariableAssignmentNode>(location(), identifier_, value_->clone());
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr ArgumentNode::clone() const {
    auto cloned = std::make_unique<ArgumentNode>(location(), identifier_, type_->clone());
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr BlockNode::clone() const {
    auto cloned = std::make_unique<BlockNode>(location(), clone_node_list(statements_));
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr FunctionCallNode::clone() const {
    auto cloned = std::make_unique<FunctionCallNode>(
        location(), identifier_, clone_node_list(arguments_));
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr IfNode::clone() const {
    auto cloned = std::make_unique<IfNode>(
        location(), condition_->clone(), 
        clone_node_list(then_block_), clone_node_list(else_block_));
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
AstNodePtr ConstantDeclarationNode::clone() const {
    auto cloned = std::make_unique<ConstantDeclarationNode>(location(), identifier_, value_->clone());
    if (result_type()) {
        cloned->set_result_type(result_type()->clone());
    }
    return std::move(cloned);
...
ConstantIntegerNode::ConstantIntegerNode(AstLocation loc, std::string value)
    ...
    set_result_type(std::make_unique<AstType>(location(), AstType::Primitive{PrimitiveType::CONST_INT}));
}

ConstantUnsignedIntegerNode::ConstantUnsignedIntegerNode(AstLocation loc, std::string value)
    : AstNode(std::move(loc)), value_(std::move(value)) {
    set_result_type(std::make_unique<AstType>(location(), AstType::Primitive{PrimitiveType::CONST_UINT}));
}

ConstantFloatNode::ConstantFloatNode(AstLocation loc, double value)
    : AstNode(std::move(loc)), value_(value) {
    set_result_type(std::make_unique<AstType>(location(), AstType::Primitive{PrimitiveType::CONST_FLOAT}));
}

ConstantBoolNode::ConstantBoolNode(AstLocation loc, bool value)
    : AstNode(std::move(loc)), value_(value) {
    set_result_type(std::make_unique<AstType>(location(), AstType::Primitive{PrimitiveType::BOOL}));
}
...
void ConstantIntegerNode::accept(AstVisitor& visitor) {
    visitor.visit(*this);
}
...
void ConstantFloatNode::accept(AstVisitor& visitor) {
    visitor.visit(*this);
}
...
void ConstantBoolNode::accept(AstVisitor& visitor) {
    visitor.visit(*this);
}
...
void VariableLookupNode::accept(AstVisitor& visitor) {
    visitor.visit(*this);
}

void ArgumentNode::accept(AstVisitor& visitor) const {
    visitor.visit(*this);
}

void ArgumentNode::accept(AstVisitor& visitor) {
    visitor.visit(*this);
}
