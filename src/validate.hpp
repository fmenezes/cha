...
public:
    ...
    static PrimitiveType convert_arithmetic_types(PrimitiveType left, PrimitiveType right);
    
    // Check assignment compatibility
    static bool is_assignment_compatible(PrimitiveType from, PrimitiveType to);
    
    // Check numeric comparison compatibility
    static bool is_numeric_comparison_compatible(PrimitiveType left, PrimitiveType right);
    
    // Check equality comparison compatibility
    static bool is_equality_comparison_compatible(PrimitiveType left, PrimitiveType right);
...
private:
    ...
    bool validate_variable_lookup(VariableLookupNode& node); // non-const for type setting
    bool validate_binary_op(BinaryOpNode& node); // non-const for type setting
    bool validate_function_call(FunctionCallNode& node); // non-const for type setting
