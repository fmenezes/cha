...
struct AstLocation {
    ...
    AstLocation(const std::string& f, int lb, int cb, int le, int ce)
        : file(f), line_begin(lb), column_begin(cb), line_end(le), column_end(ce) {}
...
inline std::ostream& operator<<(std::ostream& os, PrimitiveType type) {
    switch (type) {
        case PrimitiveType::UNDEF: return os << "UNDEF";
        case PrimitiveType::CONST_INT: return os << "CONST_INT";
        case PrimitiveType::INT: return os << "INT";
        case PrimitiveType::INT8: return os << "INT8";
        case PrimitiveType::INT16: return os << "INT16";
        case PrimitiveType::INT32: return os << "INT32";
        case PrimitiveType::INT64: return os << "INT64";
        case PrimitiveType::CONST_UINT: return os << "CONST_UINT";
        case PrimitiveType::UINT: return os << "UINT";
        case PrimitiveType::UINT8: return os << "UINT8";
        case PrimitiveType::UINT16: return os << "UINT16";
        case PrimitiveType::UINT32: return os << "UINT32";
        case PrimitiveType::UINT64: return os << "UINT64";
        case PrimitiveType::CONST_FLOAT: return os << "CONST_FLOAT";
        case PrimitiveType::FLOAT16: return os << "FLOAT16";
        case PrimitiveType::FLOAT32: return os << "FLOAT32";
        case PrimitiveType::FLOAT64: return os << "FLOAT64";
        case PrimitiveType::BOOL: return os << "BOOL";
...
public:
    AstType(AstLocation loc, Primitive p) : location_(std::move(loc)), type_data_(std::move(p)) {}
    AstType(AstLocation loc, Array a) : location_(std::move(loc)), type_data_(std::move(a)) {}
    AstType(AstLocation loc, Identifier i) : location_(std::move(loc)), type_data_(std::move(i)) {}
    
    const AstLocation& location() const { return location_; }
    
    bool is_primitive() const { return std::holds_alternative<Primitive>(type_data_); }
    bool is_array() const { return std::holds_alternative<Array>(type_data_); }
    bool is_identifier() const { return std::holds_alternative<Identifier>(type_data_); }
    
    const Primitive& as_primitive() const { return std::get<Primitive>(type_data_); }
    const Array& as_array() const { return std::get<Array>(type_data_); }
    const Identifier& as_identifier() const { return std::get<Identifier>(type_data_); }
...
public:
    ...
    virtual void visit(ConstantIntegerNode& node) { visit(const_cast<const ConstantIntegerNode&>(node)); }
    virtual void visit(ConstantUnsignedIntegerNode& node) { visit(const_cast<const ConstantUnsignedIntegerNode&>(node)); }
    virtual void visit(ConstantFloatNode& node) { visit(const_cast<const ConstantFloatNode&>(node)); }
    virtual void visit(ConstantBoolNode& node) { visit(const_cast<const ConstantBoolNode&>(node)); }
    virtual void visit(BinaryOpNode& node) { visit(const_cast<const BinaryOpNode&>(node)); }
    virtual void visit(VariableDeclarationNode& node) { visit(const_cast<const VariableDeclarationNode&>(node)); }
    virtual void visit(VariableAssignmentNode& node) { visit(const_cast<const VariableAssignmentNode&>(node)); }
    virtual void visit(VariableLookupNode& node) { visit(const_cast<const VariableLookupNode&>(node)); }
    virtual void visit(ArgumentNode& node) { visit(const_cast<const ArgumentNode&>(node)); }
    virtual void visit(BlockNode& node) { visit(const_cast<const BlockNode&>(node)); }
    virtual void visit(FunctionDeclarationNode& node) { visit(const_cast<const FunctionDeclarationNode&>(node)); }
    virtual void visit(FunctionCallNode& node) { visit(const_cast<const FunctionCallNode&>(node)); }
    virtual void visit(FunctionReturnNode& node) { visit(const_cast<const FunctionReturnNode&>(node)); }
    virtual void visit(IfNode& node) { visit(const_cast<const IfNode&>(node)); }
    virtual void visit(ConstantDeclarationNode& node) { visit(const_cast<const ConstantDeclarationNode&>(node)); }
};
...
public:
    ...
    virtual void accept(AstVisitor& visitor) = 0; // Non-const version for visitors that modify nodes
...
public:
    BinaryOpNode(AstLocation loc, Operator op, AstNodePtr left, AstNodePtr right)
        : AstNode(std::move(loc)), op_(op), left_(std::move(left)), right_(std::move(right)) {}
...
public:
    VariableAssignmentNode(AstLocation loc, std::string identifier, AstNodePtr value)
        : AstNode(std::move(loc)), identifier_(std::move(identifier)), value_(std::move(value)) {}
...
public:
    ArgumentNode(AstLocation loc, std::string identifier, AstTypePtr type)
        : AstNode(std::move(loc)), identifier_(std::move(identifier)), type_(std::move(type)) {}
...
public:
    FunctionDeclarationNode(AstLocation loc, std::string identifier, 
                           AstTypePtr return_type, AstNodeList arguments, AstNodeList body)
        : AstNode(std::move(loc)), identifier_(std::move(identifier)), 
...
public:
    FunctionCallNode(AstLocation loc, std::string identifier, AstNodeList arguments)
        : AstNode(std::move(loc)), identifier_(std::move(identifier)), 
...
public:
    ConstantDeclarationNode(AstLocation loc, std::string identifier, AstNodePtr value)
        : AstNode(std::move(loc)), identifier_(std::move(identifier)), value_(std::move(value)) {}
