...
protected:
    ...
    void expectCompilationFailure(const std::string& testFile, const std::string& expectedError) {
        std::string cmd = "./build/cha -ll out.ll test/integration/" + testFile;
        auto result = runCommand(cmd);
    ...
    void expectExecutionResult(const std::string& testFile, int expectedExitCode) {
        ...
        if (compileResult.exit_code == 0) {
            ...
            EXPECT_EQ(actualExitCode, expectedExitCode)
                << "Execution of " << testFile << " should return " << expectedExitCode;
        }
    }
};
...
// Control Flow Tests
TEST_F(IntegrationTest, IfPasses) {
    expectCompilationSuccess("if_passes.cha");
}
...
TEST_F(IntegrationTest, IfConditionNotBool) {
    expectCompilationFailure("if_condition_not_bool.cha", "condition should return bool");
}
...
TEST_F(IntegrationTest, ValidationArgDup) {
    expectCompilationFailure("validation_arg_dup.cha", "argument 'i' already defined");
}

TEST_F(IntegrationTest, ValidationArgMismatch) {
    expectCompilationFailure("validation_arg_mismatch.cha", "function 'test' expects no arguments");
}

TEST_F(IntegrationTest, ValidationArgMismatch2) {
    expectCompilationFailure("validation_arg_mismatch2.cha", "function 'test' expects arguments");
}

TEST_F(IntegrationTest, ValidationArgMismatch3) {
    expectCompilationFailure("validation_arg_mismatch3.cha", "type mismatch expects 'uint8' passed 'int'");
}

// Validation Tests - Function Errors
TEST_F(IntegrationTest, ValidationDupFunction) {
    expectCompilationFailure("validation_dup_function.cha", "'test' already defined");
}

TEST_F(IntegrationTest, ValidationFunctionNotFound) {
    expectCompilationFailure("validation_function_not_found.cha", "function 'test' not found");
}

TEST_F(IntegrationTest, ValidationRetMismatch) {
    expectCompilationFailure("validation_ret_mismatch.cha", "return type mismatch expects 'int' passed 'uint8'");
}

// Validation Tests - Type Errors
TEST_F(IntegrationTest, ValidationTypeMismatch) {
    expectCompilationFailure("validation_type_mismatch.cha", "type mismatch expects 'int' passed 'uint8'");
}

TEST_F(IntegrationTest, ValidationTypeMismatch2) {
    expectCompilationFailure("validation_type_mismatch2.cha", "incompatible types found for operation: 'int', 'uint8'");
}
...
TEST_F(IntegrationTest, ValidationVarRedefined) {
    expectCompilationFailure("validation_var_redefined.cha", "variable 'a' already defined");
}

TEST_F(IntegrationTest, ValidationVarRedefined2) {
    expectCompilationFailure("validation_var_redefined2.cha", "variable 'a' already defined");
}

// Binary/Execution Tests
TEST_F(IntegrationTest, TestBin) {
    // Note: Binary generation is not yet implemented in the compiler
    // This test verifies that compilation succeeds, even though execution isn't possible
    std::string compileCmd = "./build/cha -o out test/integration/test_bin.cha";
    auto compileResult = runCommand(compileCmd);
    
    // The compiler currently produces an object file instead of an executable binary
    // So we expect compilation to "succeed" but with a warning message
    EXPECT_EQ(compileResult.exit_code, 0) 
        << "Compilation of test_bin.cha should succeed (even if binary generation is not implemented)\n"
        << "Command: " << compileCmd << "\n"
        << "Output: " << compileResult.output;
